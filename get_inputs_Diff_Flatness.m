...
% 
    ...
    t(:,3)=t(:,3)+(ones(size(t,1),1)*data.g);
    ...
    x_c=[cos(out(:,4,1)), sin(out(:,4,1)),zeros(size(out,1),1)]; % x unit vector of intermediate frame (only rotated for yaw)
    y_c=[-sin(out(:,4,1)), cos(out(:,4,1)),zeros(size(out,1),1)];
    y_b=cross(z_b,x_c,2);
 ...
 %TODO may have made errors when mutliplying with z_b 
    ...
    gzw=zeros(size(z_b));
    gzw(:,3)=data.g;
    c2=dot(z_b,out(:,1:3,3)+gzw,2); %mindcheck
...
%         for s =1:size(term_1,3)
            term_1(i,:)=(I*omega_dot(i,:)')';
            term_2(i,:)=(I*omega_bw(i,:)')';
%         end
    end
